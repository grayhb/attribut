...
function Plugin( frame, o ){
	 ...
	 * @param {Element|Int} el DOM element, or index of element in items array
	 */
	function getIndex( el , s = ""){

		//return isNumber(el) ? el < 0 ? 0 : el > items.length-1 ? items.length-1 : el : el === undefined ? -1 : $items.index( el );
		//мой фикс для бесконечной прокрутки:
		//console.log(isNumber(el) ? el < 0 ? items.length-1 : el > items.length-1 ? 0 : el : el === undefined ? -1 : $items.index( el ))
		if (s=="") 
			return isNumber(el) ? el < 0 ? items.length-1 : el > items.length-1 ? 0 : el : el === undefined ? -1 : $items.index( el );
		 else
			return isNumber(el) ? el < 0 ? 0 : el > items.length-1 ? items.length-1 : el : el === undefined ? -1 : $items.index( el );
	...
	(function(){
		...
		o.scrollBy && $scrollSource.bind('DOMMouseScroll.' + namespace + ' mousewheel.' + namespace, function(e){
			...
			if( itemNav ){
				...
				nextItem = getIndex( ( centeredNav ? forceCenteredNav ? rel.activeItem : rel.centerItem : rel.firstItem ) + ( isForward ? o.scrollBy : -o.scrollBy ), 1 );
